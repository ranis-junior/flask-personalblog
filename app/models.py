...
# db.event.listen(db.session, 'before_flush', teste)
# db.event.listen(db.session, 'after_flush', teste)
...
class User(db.Model, UserMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.email_address and not self.avatar_hash:
    ...
    def new_messages(self):
        last_read_time = self.last_message_read_time or datetime.min
    ...
    def gravatar_hash(self):
        return md5(self.email_address.lower().encode('utf-8')).hexdigest()
    ...
    def get_reset_password_token(self, expire=600):
        return jwt.encode({'reset_password': self.id, 'exp': time() + expire},
                          current_app.config['SECRET_KEY']).decode('utf-8')

    @staticmethod
    def verify_reset_password_token(token):
        try:
            id = jwt.decode(token, current_app.config['SECRET_KEY'])['reset_password']
        except:
            return
